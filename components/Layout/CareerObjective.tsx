        ...
        <div className="relative flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="space-y-4 flex-1">
            ...
            <div className="flex flex-wrap gap-3 pt-4">
              {[
                { label: "Precise", color: "teal" },
                { label: "Automated", color: "blue" },
                { label: "Linux-Optimized", color: "emerald" }
              ].map((item, i) => (
                <span 
                  key={i} 
                  className={`px-4 py-2 text-sm font-medium rounded-full bg-${item.color}-50/80 border border-${item.color}-200/50 text-${item.color}-700 backdrop-blur-sm`}
                >
                  {item.label}
                </span>
              ))}
